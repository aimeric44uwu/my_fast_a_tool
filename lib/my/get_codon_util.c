...
static void is_a_start(char *content, codon_t *codon_struct)
{
    ...
    if (content[j] == 'A' && content[j + 1] == 'T' && content[j + 2] == 'G'
        && strncmp(&content[j + 3], "ATG", 3) != 0) {
        if (codon_struct->start != -1 &&
            strncmp(&content[j + 5], "ATG", 3) != 0 ) {
...
{
    int end = codon_struct->end;
    int start = codon_struct->start;

    if (end != -1 && start != -1 && (end - start) >= 6) {
        codon[codon_struct->count] = new_array(end - start);
        parse_between(codon[codon_struct->count], content, codon_struct);
        codon[codon_struct->count++][end - start] = '\0';
        codon_struct->start = -1;
        codon_struct->end = -1;
    }
}

int get_codon_ptwo(char *content, char **codon, int count)
{
